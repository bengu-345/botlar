from ayarlar import token_sozlugu
import discord
...
async def on_message(message):
    ...
    if message.content.startswith('$hello'):
        ...
        await message.channel.send(bot_mantik.hosgeldiniz_mesaji())
    elif message.content.startswith('$pass'):
        await message.channel.send(bot_mantik.sifre_olusturucu(10))
    else:
        await message.channel.send("Bu komutu anlayamadım :(")

istemci.run(token_sozlugu["TOKEN"])
